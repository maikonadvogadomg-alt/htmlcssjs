...
export class DatabaseStorage implements IStorage {
  ...
  async setSetting(key: string, value: string): Promise<void> {
    ...
    try {
      ...
      if (!isAiKey(key)) throw new Error("Banco de dados indisponÃ­vel. Configure a URL do banco nas ConfiguraÃ§Ãµes.");
    }
  }
...
export async function checkDbAndInitStorage(): Promise<boolean> {
  ...
  try {
    ...
    client.release();
    _dbAvailable = true;
    console.log("[storage] Banco de dados conectado â usando PostgreSQL");
  } catch (e: any) {
    _dbAvailable = false;
    _backend = new MemoryStorage();
    console.warn("[storage] Banco indisponÃ­vel â usando memÃ³ria. Configure o banco na tela de ConfiguraÃ§Ãµes.");
  }
  return _dbAvailable;
}
