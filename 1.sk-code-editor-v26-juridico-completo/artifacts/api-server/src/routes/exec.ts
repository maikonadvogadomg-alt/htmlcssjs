...
// âââ POST /api/projects/:projectId/exec-stream âââââââââââââââââââââââââââââââ
// Executa um comando via SSE streaming â sem WebSocket, sem reconexÃ£o.
// O cliente faz fetch POST e lÃª o stream SSE linha a linha.
// Eventos: stdout | stderr | server_detected | exit
const SERVER_PORT_RE_EXEC = /(?:listen(?:ing)?(?:\s+on)?(?:\s+port)?|started(?:\s+on)?|running(?:\s+on)?(?:\s+port)?|\bport\b|localhost|127\.0\.0\.1|0\.0\.0\.0|Local:|Network:|â)\s*[:\s]*(?:https?:\/\/(?:localhost|127\.0\.0\.1))?:(\d{4,5})/i;
...
router.post("/projects/:projectId/exec-stream", (req, res) => {
  ...
  // Cria diretÃ³rio persistente de projetos
  ...
  // Filtra variÃ¡veis do pnpm/workspace que geram warnings indesejados no npm
  ...
  // Usa res.on('close') â nÃ£o req: req.close dispara quando o body POST Ã© recebido
  res.on("close", () => {
