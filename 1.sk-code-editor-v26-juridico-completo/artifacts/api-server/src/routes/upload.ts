...
async function extractText(buffer: Buffer, mimetype: string, filename: string): Promise<string> {
  ...
  if (ext === "pdf" || mimetype === "application/pdf") {
    try {
      ...
      // fallback: extraÃ§Ã£o simples por regex
...
// Campo "files" (mÃºltiplos) ou "file" (Ãºnico)
router.post(
  "/upload/extract-text",
  ...
  async (req, res) => {
    try {
      ...
      if (parts.length === 0) {
        res.status(422).json({ message: "NÃ£o foi possÃ­vel extrair texto dos arquivos enviados." });
        return;
      }
