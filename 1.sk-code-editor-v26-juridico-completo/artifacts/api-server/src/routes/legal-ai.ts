...
const ACTION_PROMPTS: Record<string, string> = {
  ...
  minuta:      "Elabore PETICAO/MINUTA JURIDICA COMPLETA, EXTENSA E PROFISSIONAL com NO MINIMO 15 PAGINAS (7.500+ palavras). Inclua OBRIGATORIAMENTE todas as secoes: EndereÃ§amento â QualificaÃ§Ã£o das partes â Dos Fatos (extenso, cronologico, minimo 8 paragrafos) â Do Direito (fundamentacao robusta, minimo 12 paragrafos, multiplas teses) â Da Jurisprudencia (cite precedentes) â Dos Pedidos (lista detalhada, minimo 8 pedidos) â Do Valor da Causa â Assinatura.\n\nATENCAO: MINIMO 15 PAGINAS COMPLETAS. PROIBIDO rascunho curto.\n\nINFORMACOES:\n{{texto}}",
...
async function streamGeminiBuiltin(
  ...
  if (!baseUrl || !apiKey) throw new Error("IntegraÃ§Ã£o de IA nÃ£o configurada no servidor.");
...
// âââ POST /api/legal/process ââââââââââââââââââââââââââââââââââââââââââââââââââ
// Processa texto jurÃ­dico com aÃ§Ã£o especÃ­fica (streaming SSE)
...
router.post("/legal/process", async (req, res) => {
  try {
    ...
    if (!text?.trim()) {
      res.status(400).json({ message: "Texto Ã© obrigatÃ³rio." });
      return;
    }
    ...
    // Aceita prompt customizado (aÃ§Ãµes do usuÃ¡rio)
    const { customPrompt } = req.body as { customPrompt?: string };
    const template = customPrompt
      ? customPrompt + "\n\nTEXTO:\n{{texto}}"
      : ACTION_PROMPTS[action];
    if (!template) {
      res.status(400).json({ message: `AÃ§Ã£o desconhecida: ${action}` });
      return;
    }
    ...
    if (cleanKey) {
      // Modo Chave PrÃ³pria â tudo server-side, sem expor dados no browser
      ...
      // Modo Demo â usa integraÃ§Ã£o Gemini/OpenAI do servidor (gratuita)
...
// âââ POST /api/legal/refine âââââââââââââââââââââââââââââââââââââââââââââââââââ
...
router.post("/legal/refine", async (req, res) => {
  try {
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
      res.status(400).json({ message: "HistÃ³rico de mensagens obrigatÃ³rio." });
      return;
    }
