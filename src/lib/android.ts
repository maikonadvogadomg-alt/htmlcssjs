...
export function genReadme(c: AppConfig, source: string) {
  return `# ${c.appName} â Projeto Android (Capacitor)
...
## ConfiguraÃ§Ã£o
- **Package:** \`${c.appId}\`
- **VersÃ£o:** ${c.versionName} (code: ${c.versionCode})
- **Min SDK:** Android ${c.minSdk}+
- **OrientaÃ§Ã£o:** ${c.orientation}
`;
}

/* ââ package.json para Capacitor + npm install ââââââââââââââââ */
...
/* ââ GitHub Actions workflow â compila APK na nuvem âââââââââââ */
...
/* ââ PWA: manifest.webmanifest âââââââââââââââââââââââââââââââ */
...
/* ââ PWA: sw.js âââââââââââââââââââââââââââââââââââââââââââââââ */
export function genServiceWorker(c: AppConfig): string {
  return `/* ${c.appName} â Service Worker v${c.versionName} */
...
/* ââ PWA: injeta metatags no index.html âââââââââââââââââââââââ */
...
/* ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
   EAS (Expo Application Services) â build local via GitHub Actions
   NÃ£o precisa de conta Expo nem EXPO_TOKEN.
...
module.exports = function withCopyWww(config) {
  return withDangerousMod(config, ['android', (config) => {
    ...
    if (fs.existsSync(src)) {
      copyDir(src, dst);
      console.log('[withCopyWww] Copiado www/ â', dst);
    } else {
      console.warn('[withCopyWww] Pasta www/ nÃ£o encontrada em', src);
    }
    return config;
  }]);
};
...
/** Workflow EAS CLOUD â usa a conta Expo do usuÃ¡rio (requer EXPO_TOKEN como secret) */
    ...
    steps:
      ...
      - name: Aguardar e baixar APK do EAS
        run: |
          ...
          if [ -z "$BUILD_ID" ]; then echo "Build ID nÃ£o encontrado, verificando localmente..."; exit 0; fi
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Expo Prebuild (gera cÃ³digo Android nativo)
...
export async function buildAndroidZip(
  ...
  // Processa web files uma vez â injeta PWA no index.html
  const dec = new TextDecoder();
  let hasIndex = false;
  ...
  // dist/ â para referÃªncia e cap copy
  const dist = zip.folder("dist")!;
  // android/app/src/main/assets/public/ â para build direto (GitHub Actions)
