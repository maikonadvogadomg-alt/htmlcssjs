...
export default function Pdpj() {
  ...
  async function testar() {
    ...
    try {
      ...
      if (resp.ok) {
        setStatus("ok");
        setMsgStatus(`ConexÃ£o OK â Status ${resp.status}`);
      } else {
        setStatus("erro");
        setMsgStatus(`Erro ${resp.status} â Token invÃ¡lido ou expirado`);
      }
    } catch (e: any) {
      setStatus("erro");
      setMsgStatus(e.message || "Erro de conexÃ£o");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function limpar() {
    Alert.alert("Limpar token?", "O token salvo serÃ¡ removido.", [
      { text: "Cancelar" },
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>ð¡ï¸ PDPJ â Token de Acesso</Text>
        <Text style={styles.sub}>Configure seu token JWT para o Portal Digital do Poder JudiciÃ¡rio</Text>
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          ...
          <TextInput
            ...
            placeholder="Cole aqui seu token PDPJ (comeÃ§a com eyJ...)"
        ...
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>â¹ï¸ Como obter o token PDPJ</Text>
          <Text style={styles.infoStep}>1. Acesse pdpj.jus.br com seu certificado digital</Text>
          <Text style={styles.infoStep}>2. VÃ¡ em Perfil â Gerar Token JWT</Text>
          <Text style={styles.infoStep}>3. Copie o token completo e cole aqui</Text>
          <Text style={styles.infoStep}>4. O token expira em 24h â renove quando necessÃ¡rio</Text>
        </View>

        <View style={[styles.infoCard, { borderColor: "#2d3a4a" }]}>
          <Text style={styles.infoTitle}>ð SeguranÃ§a</Text>
          <Text style={styles.infoText}>O token Ã© salvo APENAS no seu dispositivo (AsyncStorage local). NÃ£o Ã© enviado a nenhum servidor externo â sÃ³ ao PDPJ oficial quando vocÃª usa as funcionalidades.</Text>
        </View>
      </ScrollView>
    </View>
  );
