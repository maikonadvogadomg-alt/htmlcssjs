...
export default function ConfigInicial() {
  ...
  async function concluir() {
    ...
    try {
      ...
      Alert.alert("Erro", "NÃ£o foi possÃ­vel salvar. Tente novamente.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: COLORS.bg }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.logoBox}>
          ...
          <Text style={styles.logoTitle}>Assistente JurÃ­dico</Text>
          <Text style={styles.logoSub}>ConfiguraÃ§Ã£o inicial</Text>
        </View>
        ...
        {passo === 1 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>ð SeguranÃ§a do App</Text>
            <Text style={styles.cardDesc}>Defina uma senha para proteger o app. VocÃª pode pular esta etapa se preferir acesso direto.</Text>
            ...
            {usarSenha && (
              <>
                <TextInput style={styles.input} placeholder="Senha (mÃ­nimo 4 caracteres)" placeholderTextColor={COLORS.textDim} value={senha} onChangeText={setSenha} secureTextEntry autoCapitalize="none" />
        ...
        {passo === 2 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>ð¤ Chaves de IA</Text>
            ...
            {chave1Info && <Text style={styles.detected}>â Detectado: {chave1Info.nome} â {chave1Info.modelo}</Text>}

            <Text style={styles.label}>Chave SecundÃ¡ria (Chave 2) â opcional</Text>
            ...
            {chave2Info && <Text style={styles.detected}>â Detectado: {chave2Info.nome} â {chave2Info.modelo}</Text>}

            <View style={styles.infoBox}>
              <Text style={styles.infoTitle}>Provedores suportados:</Text>
              {[
                ["gsk_...", "Groq â GRATUITO (llama-3.3-70b)"],
                ["AIza...", "Google Gemini â GRATUITO"],
                ["sk-or-...", "OpenRouter â modelos variados"],
                ["sk-ant...", "Anthropic Claude"],
                ["pplx-...", "Perplexity (busca web)"],
                ["sk-...", "OpenAI GPT-4o"],
              ].map(([prefix, desc]) => (
                <Text key={prefix} style={styles.infoItem}>â¢ <Text style={styles.infoPrefix}>{prefix}</Text> {desc}</Text>
              ))}
            </View>
          </View>
        )}
        ...
        {passo === 3 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>ðï¸ Banco de Dados (opcional)</Text>
            ...
            <View style={styles.infoBox}>
              ...
              <Text style={styles.infoItem}>2. Crie um projeto â copie a "Connection string"</Text>
              <Text style={styles.infoItem}>3. Cole aqui acima</Text>
              <Text style={[styles.infoItem, { marginTop: 8, color: COLORS.textMuted }]}>
                Sem banco: histÃ³rico e snippets ficam salvos localmente. Funciona normalmente.
              </Text>
            </View>
          </View>
        )}
        ...
        <TouchableOpacity style={styles.btnPrimary} onPress={concluir} disabled={loading}>
          <Text style={styles.btnPrimaryText}>{loading ? "Salvando..." : passo < 3 ? "PrÃ³ximo â" : "Concluir ConfiguraÃ§Ã£o"}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.btnPular} onPress={pularConfig}>
          <Text style={styles.btnPularText}>Pular configuraÃ§Ã£o</Text>
        </TouchableOpacity>
      </ScrollView>
